...
public class CountermeasureDegree1 {
    ...
    public CountermeasureDegree1(int dataset, double beta, double gamma, double per, double[] epsilon_all, String attackType) throws Exception {
        this.dataset = dataset;
        this.beta = beta;
        this.gamma = gamma;
        CountermeasureDegree1.attackType = attackType;

        CountermeasureDegree1.percentageForMatrix = Tools.optimalPercentage[dataset - 1][metric - 1][(int) epsilon - 1];
    ...
    public void testDegreeEstimation_list(boolean[][] mat, double epsilon, int type) throws Exception {
        ...
        double[] degreeFromMatrix = Tools.getDegree(mergedMat);
        noiseDegree = Tools.addLaplaceNoise_Degree(noiseDegree, epsilon * (1.0 - percentage));
        degreeFromMatrix = Tools.addLaplaceNoise_Degree(degreeFromMatrix, epsilon * (1.0 - percentageForMatrix));

        int maxDegree = 0;
        for (double degree : degreeFromMatrix) {
        ...
        switch (attackType) {
            case "randomValueAttack":
                for (int node : fakeNodes) {
                    degreeFromMatrix[node] = (int) (Math.random() * (totalNum));
                }
        }

        detectAndEvaluateFakeNodes(mergedMat, degreeFromMatrix, noiseDegree, isFakeNode, attackerNum, percentage, epsilon, isTargetNode);

        String baseDir = "output" + File.separator + "counter_degree_degree" + File.separator + attackType + File.separator + dataset;
        File dir = new File(baseDir);
        if (!dir.exists() && !dir.mkdirs()) {
    ...
    private void detectAndEvaluateFakeNodes(boolean[][] adjacencyMat, double[] degreeFromMatrix, double[] noiseDegree, boolean[] isFakeNode, int actualFakeNodesCount, double percentage, double epsilon, boolean[] isTargetNode) {
        ...
        double maxNoiseDegree = Double.MIN_VALUE;
        for (double degree : noiseDegree) {
            if (degree > maxNoiseDegree) {
                maxNoiseDegree = degree;
            }
        }

        double detectionThreshold = maxNoiseDegree + threshold;

        for (int i = 0; i < totalNum; i++) {
            if (Math.abs(degreeFromMatrix[i] - noiseDegree[i]) > detectionThreshold && !isTargetNode[i]) {
                potentialFakeNodes[i] = true;
                potentialFakeNodeCount++;
            }
        }

        int[] targetFakeConnections = new int[totalNum];
        for (int i = 0; i < totalNum; i++) {
            if (isTargetNode[i]) {
                for (int j = 0; j < totalNum; j++) {
                    if (potentialFakeNodes[j] && adjacencyMat[i][j]) {
                        targetFakeConnections[i]++;
                    }
                }
            }
        }
        ...
        for (int i = 0; i < totalNum; i++) {
            if (isTargetNode[i]) {
                noiseDegree[i] -= targetFakeConnections[i];
                noiseDegree[i] = Math.max(0, noiseDegree[i]);
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        double[] beta_values = {0.001, 0.005, 0.01, 0.05, 0.1, 0.15};

        for (double beta : beta_values) {
            System.out.println("Running experiment with beta = " + beta);
            new CountermeasureDegree1(dataset, beta, defaultGamma, fixPercentage, fixedEpsilon, attackTypes[0]);
        }
    }
}
