...
public class AttackLCC {
    ...
    public AttackLCC(int dataset, double beta, double gamma, double per, double[] epsilon_all, String attackType) throws Exception {
        this.dataset = dataset;
        this.beta = beta;
        this.gamma = gamma;
        AttackLCC.attackType = attackType;

        AttackLCC.percentageForMatrix = Tools.optimalPercentage[dataset - 1][metric - 1][(int) epsilon - 1];
    ...
    private void calculateEdgeDensity() {
        long edgeCount = 0;
        long totalNum = this.totalNum;
        long possibleEdges = totalNum * (totalNum - 1) / 2;
    ...
    public void writeRealCoefficientToFile() throws Exception {
        double[] coefficient = com.graph.metric.ClusteringCoefficient.getLocalClusteringCoefficientList(this.mat);
        ...
        if (type == 8) {
            switch (attackType) {
                ...
                case "maximumGainAttack":
                    ...
                    break;
                case "untargetedAttack":
    Random untargetedRandom = new Random(SEED);
    int totalAddedUntargetedEdges = 0;

    for (int fakeNode : fakeNodes) {
        List<Integer> allPossibleNodes = new ArrayList<>();
        for (int i = 0; i < totalNum; i++) {
            if (i != fakeNode && !fakeMat[fakeNode][i]) {
                allPossibleNodes.add(i);
            }
        }
        Collections.shuffle(allPossibleNodes, untargetedRandom);

        for (int target : allPossibleNodes) {
            fakeMat[fakeNode][target] = true;
            fakeMat[target][fakeNode] = true;
            totalAddedUntargetedEdges++;
        }
    }

    System.out.println("UntargetedAttack: Added " + totalAddedUntargetedEdges + " edges (max possible per fake node)");
    break;
            }
        }
        ...
        String baseDir = "output" + File.separator + "attack" + File.separator + attackType + File.separator + dataset;
    ...
    public static void main(String[] args) throws Exception {
        ...
        String[] attackTypes = {"randomValueAttack", "randomNodeAttack", "maximumGainAttack", "untargetedAttack"};
        ...
        for (String attackType : attackTypes) {
            ...
            for (double epsilon : epsilonAll) {
                ...
                new AttackLCC(dataset, defaultBeta, defaultGamma, fixPercentage, new double[]{epsilon}, attackType);
            }

            for (double beta : beta_values) {
                System.out.println("Running experiment with beta = " + beta);
                new AttackLCC(dataset, beta, defaultGamma, fixPercentage, fixedEpsilon, attackType);
            }

            for (double gamma : gamma_values) {
                System.out.println("Running experiment with gamma = " + gamma);
                new AttackLCC(dataset, defaultBeta, gamma, fixPercentage, fixedEpsilon, attackType);
            }
        }
    }
}